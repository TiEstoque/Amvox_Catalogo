...
export default async function handler(req, res) {
  try {
    ...
    if (req.method === 'PATCH') {
      ...
      const { categoria, numero, titulo, descricao, preco, condicao } = body;
      ...
      const { error } = await supabase
        .from('items')
        .update({ categoria, numero: String(numero), titulo, descricao: descricao || '—', condicao: condicao || null, preco })
        .eq('id', id);
